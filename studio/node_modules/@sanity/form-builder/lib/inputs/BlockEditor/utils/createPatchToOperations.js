...
function createPatchesToChange(blockContentFeatures, blockContentType) {
  ...
  function patchSpanText(patch, editor, node, blockNode) {
    var textPath = editor.value.document.assertPath(node.key);
    var blockKey = patch.path[0]._key;
    var childKey = findLastKey(patch.path);
    var workTextNode = node.toJSON({
      preserveKeys: true
    });
    var blockIndex = getBlockTextIndex(blockNode, node);
    var leafIndex = workTextNode.leaves.findIndex((leaf, index) => `${blockKey}${blockIndex + index}` === childKey); // Insert patches for new spans
  ...
  return function patchToOperations(patch, editorValue) {
    ...
    if (patch.path.length > 1) {
      if (patch.path[1] === 'markDefs') {
        ...
        var rootBlockNode = controller.value.document.getNode(patch.path[0]._key);
        var node = findLastKnownEditorNodeInPath(rootBlockNode, patch.path);
        var isVoid = controller.query('isVoid', node); // If it is a unset patch, just remove the node normally and return
        ...
        if (patch.type === 'insert' || patch.type === 'set' || patch.type === 'diffMatchPatch') {
          return patchSpanText(patch, controller, findTextNodeFromPathKey(rootBlockNode, findLastKey(patch.path)), rootBlockNode);
        }
