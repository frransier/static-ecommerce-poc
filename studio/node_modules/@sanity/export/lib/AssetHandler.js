  ...
  downloadAsset(assetDoc, dstPath) {
    ...
    return _asyncToGenerator(function* () {
      ...
      var _ref5 = yield writeHashedStream(tmpPath, stream),
          sha1 = _ref5.sha1,
          md5 = _ref5.md5,
          size = _ref5.size; // Verify it against our downloaded stream to make sure we have the same copy
      ...
      if (differs && attemptNum < 3) {
        ...
        var details = [method === 'md5' ? `md5 should be ${remoteMd5}, got ${md5}` : `sha1 should be ${remoteSha1}, got ${sha1}`, contentLength && parseInt(contentLength, 10) !== size && `Image should be ${contentLength} bytes, got ${size}`, `Did not succeed after ${attemptNum} attempts.`];
        var detailsString = `Details:\n - ${details.filter(Boolean).join('\n - ')}`;
        yield fse.unlink(tmpPath);

        _this2.queue.clear();

        var error = new Error(`Failed to download image at ${assetDoc.url}, giving up. ${detailsString}`);
