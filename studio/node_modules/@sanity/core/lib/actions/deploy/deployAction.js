...
function _checkDir() {
  _checkDir = _asyncToGenerator(function* (sourceDir) {
    try {
      ...
      var error = err.code === 'ENOENT' ? new Error(`Directory "${sourceDir}" does not exist`) : err;
      throw error;
    }
    ...
    try {
      ...
      var _error = err.code === 'ENOENT' ? new Error([`"${sourceDir}/index.html" does not exist -`, '[SOURCE_DIR] must be a directory containing', 'a Sanity studio built using "sanity build"'].join(' ')) : err;
