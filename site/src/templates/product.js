...
const ProductTemplate = props => {
  ...
  return (
    <Layout>
      ...
      <h1>{state.title}</h1>
      <div>Article no: {state.articleNo}</div>
      <br></br>
      <div style={flex}>
        <div>
          <div>Standard price: {state.standard} kr</div>
          <div>Discount price: {state.discount} kr</div>
          <div>Club jaktia price: {state.jaktia} kr</div>
        </div>
        <div>
      ...
      <div style={flex}>
        {product.thumbnails.map((image, index) => (
          <div
            key={index}
            onClick={() =>
              dispatch({ type: "set-mainImage", image: product.images[index] })
            }
          >
